...
async function handleSetToken(value) {
    SecureStore.setItemAsync(SecureStoreEnum.TOKEN, value).then;
    setToken(value);
};
  ...
  const handleGetToken = async () => {
    ...
    const handleSubmitPress = () => {
        ...
        fetch("http://" + ip_address + "/v1/users/login", {
            method: "PUT",
            body: JSON.stringify({
                email: address,
                password: pass
            ...
            .then((response) => {
                //Hide Loader
                setLoading(false);
                let loggedEmail = JSON.stringify(response["email"]);
                let loggedPass = JSON.stringify(response["password"]);
                // checking if returned response matches input
                if (loggedEmail.length > 0 ) {
                    ...
                    AsyncStorage.setItem(
                        "lName",
                        JSON.stringify(response["lastName"])
                    );
                    AsyncStorage.setItem(
                        "userLocation",
                        JSON.stringify(response["city"])
                    );

                    handleSetToken(JSON.stringify(response["id"]));
    ...
    return (
    <KeyboardAwareScrollView style={{backgroundColor: theme.splash, flex:1}}>
 ...
 <View style={{
       ...
       <Text  style = {{paddingTop: "8%",paddingBottom: "2%",fontSize: 24,fontWeight: '500',color: theme.settingsGeneral,}}>Log In</Text>

       <Text  style = {{paddingTop: "5%",paddingBottom: "2%",fontSize: 14,fontWeight: '400',color: theme.settingsGeneral,}}>Your Email</Text>
        <View style={{
          flexDirection: 'row',
        ...
        <View style={{paddingHorizontal: 12,
           alignItems: 'center',}}>
            <Svg width="24" height="20">
                <Path d="M21 1H3C1.89543 1 1 1.89543 1 3V17C1 18.1046 1.89543 19 3 19H21C22.1046 19 23 18.1046 23 17V3C23 1.89543 22.1046 1 21 1Z" stroke={theme.settingsGeneral} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                <Path d="M5 5L12 11L19 5" stroke={theme.settingsGeneral} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </Svg>
        </View>
        <TextInput
            style={{flex: 1,
          ...
          <HelperText style={{color: theme.errorText}} type="error" visible={hasErrors()}>
            Email address is invalid!
          </HelperText>


       <Text  style = {{paddingTop: "8%",paddingBottom: "2%",fontSize: 14,fontWeight: '400',color: theme.settingsGeneral,}}>Your Password</Text>
        <View style={{
          flexDirection: 'row',
          justifyContent: 'center',
          alignItems: 'center',
        ...
        <View style={{paddingHorizontal: 12,
           alignItems: 'center',}}>
            <Svg width="24" height="24">
                <Path d="M17 13L19 15L23 11L21 9" stroke={theme.settingsGeneral} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                <Path d="M11.2422 12.758L23.0002 1" stroke={theme.settingsGeneral} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                <Path d="M7 23C10.3137 23 13 20.3137 13 17C13 13.6863 10.3137 11 7 11C3.68629 11 1 13.6863 1 17C1 20.3137 3.68629 23 7 23Z" stroke={theme.settingsGeneral} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </Svg>
        </View>
        <TextInput
            style={{flex: 1,
          ...
          />
                ...
                <View style={{flexDirection: 'column', paddingTop: "8%", alignItems: 'flex-end'}}>
                        ...
                        <GradientButton
                            ...
                            gradientBegin= {sport ? theme.gradientL : theme.updateButtonL}
                            gradientEnd={sport ? theme.gradientR : theme.updateButtonR}
  ...
  flexDirection: 'row'
   ...
   <Text style={{ textAlign: "center", color: theme.gradientL, fontWeight: "500", fontSize: 15 }}> New User? </Text>
                        <Text
                            style={{ paddingLeft:'1%', textAlign: "center",
                                    color: theme.gradientR,
                                    fontWeight: "500",
                                    fontSize: 15,
                                     }}
                            onPress={() =>
                                navigation.navigate("RegistrationNavigator")
                            }
                        >
                            Sign Up
                        </Text>
